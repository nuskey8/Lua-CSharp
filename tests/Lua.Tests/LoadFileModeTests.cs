...
public sealed class LoadFileModeTests : IDisposable
{
    ...
    sealed class NonSeekableByteFileSystem(byte[] bytes)
        : Helpers.NotImplementedExceptionFileSystemBase
    {
    ...
    readonly string testDirectory = Path.Combine(
        Path.GetTempPath(),
        $"LuaLoadFileModeTests_{Guid.NewGuid()}"
    );
    ...
    {
        File.WriteAllBytes(
            Path.Combine(testDirectory, "text.lua"),
            Encoding.UTF8.GetBytes("return 10")
        );

        using var state = CreateState();

        var exception = Assert.ThrowsAsync<Exception>(async () =>
            await state.LoadFileAsync("text.lua", "b", null, CancellationToken.None)
        );
    ...
    {
        File.WriteAllBytes(
            Path.Combine(testDirectory, "binary.luac"),
            [0x1B, (byte)' ', (byte)'r', (byte)'e', (byte)'t', (byte)'u', (byte)'r', (byte)'n']
        );

        using var state = CreateState();

        var exception = Assert.ThrowsAsync<Exception>(async () =>
            await state.LoadFileAsync("binary.luac", "t", null, CancellationToken.None)
        );
    ...
    public async Task LoadFile_NonSeekableByteStream_TextChunk_DoesNotRequireSeek()
    {
        ...
        {
            FileSystem = new NonSeekableByteFileSystem(Encoding.UTF8.GetBytes("return 10")),
        };
