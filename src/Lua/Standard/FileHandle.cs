...
{
    public static readonly LuaFunction IndexMetamethod = new(
        "index",
        (context, ct) =>
        {
    ...
    public FileHandle(Stream stream, LuaFileOpenMode mode)
        : this(ILuaStream.CreateFromStream(stream, mode)) { }
    ...
    static readonly LuaFunction CloseFunction = new(
        "close",
    ...
    static readonly LuaFunction FlushFunction = new(
        "file.flush",
        async (context, cancellationToken) =>
        {
            var file = context.GetArgument<FileHandle>(0);
    ...
    static readonly LuaFunction LinesFunction = new(
        "file.lines",
        (context, cancellationToken) =>
        {
            var file = context.GetArgument<FileHandle>(0);
            var format = context.HasArgument(1) ? context.Arguments[1] : "*l";
                ...
                context.Return(
                    new CSharpClosure(
                        ...
                        static async (context, cancellationToken) =>
                        {
                            ...
                            context.Return();
                            var resultCount = await IOHelper.ReadAsync(
                                context.State,
                                file,
                                "file.lines",
                                0,
    ...
    static readonly LuaFunction ReadFunction = new(
        "file.read",
        async (context, cancellationToken) =>
        {
            var file = context.GetArgument<FileHandle>(0);
    ...
    static readonly LuaFunction SeekFunction = new(
        "file.seek",
        (context, cancellationToken) =>
        {
            var file = context.GetArgument<FileHandle>(0);
            var whence = context.HasArgument(1) ? context.GetArgument<string>(1) : "cur";
            var offset = context.HasArgument(2) ? context.GetArgument<int>(2) : 0;

            if (whence is not ("set" or "cur" or "end"))
            {
                throw new LuaRuntimeException(
                    context.State,
                    $"bad argument #2 to 'file.seek' (invalid option '{whence}')"
                );
            }
    ...
    static readonly LuaFunction SetVBufFunction = new(
        "file.setvbuf",
        (context, cancellationToken) =>
        {
            var file = context.GetArgument<FileHandle>(0);
            var mode = context.GetArgument<string>(1);
            var size = context.HasArgument(2) ? context.GetArgument<int>(2) : -1;
    ...
    static readonly LuaFunction WriteFunction = new(
        "file.write",
        async (context, cancellationToken) =>
        {
            var file = context.GetArgument<FileHandle>(0);
            var resultCount = await IOHelper.WriteAsync(
                file,
                "io.write",
                context with
                {
    ...
    static readonly LuaFunction ToStringFunction = new(
        "file.__tostring",
        (context, cancellationToken) =>
        {
            var file = context.GetArgument<FileHandle>(0);
            return new(
                context.Return($"file ({(file.IsOpen ? file.stream.GetHashCode() : "closed")})")
            );
        }
    );
}
